...
import { Divider, Input, Button, Select, Radio, App, Form, Typography, message } from 'antd';
...
const Word: React.FC = () => {
  ...
  const [showCorpusTable, setShowCorpusTable] = useState(false);
  ...
  const handleSearch = () => {
    if (rows_1.length === 0 || rows_2.length === 0) {
      appMessage.warning(t('missing_data'));
      return;
    }
    else if (!searchText.trim()) {
      appMessage.warning(t('missing_input'))
      return;
    }
  ...
  const handleSaveButton = () => {
    ...
    appMessage.success(t('download_ready'));
  }

  const handleWordSelect = (word: RowWord) => {
    setSearchText(word.Word);
    setShowCorpusTable(false);
  };
        ...
        <div className="p-3">
          ...
          >
            ...
            <Form.Item>
              <Button 
                type="default" 
                onClick={() => setShowCorpusTable(!showCorpusTable)}
              >
                {showCorpusTable ? 'Hide Corpus' : 'Show Corpus'}
              </Button>
            </Form.Item>
                      </Form>
            ...
            {showCorpusTable && (
              <div style={{ marginTop: 16 }}>
                <CorpusTable onWordSelect={handleWordSelect} />
              </div>
            )}
